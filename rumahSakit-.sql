...
from ida.customers 
where contactLastName = 'Andersen'

select customerName,creditlimit*0.95  as zzzzz from ida.customers 

select max(creditlimit) as highest  from ida.customers

select customerName, addressline1 from ida.customers where addressline1 like '%Road%'

select customername, postalcode from ida.customers where postalCode like '_5%'

select customername, creditlimit from ida.customers where creditLimit > 100000

select customername, state from ida.customers where state is nulladdressLine1

select customername, country, creditlimit from ida.customers where (country = 'Germany' or country = 'Spain') and creditlimit <= 50000

show tables

select * from ida.employees limit 5

select e.lastName, e.jobTitle, o.city, o.country from  ida.employees e join ida.offices o on e.officecode

select * from ida.orders limit 5

select c.customername, c.phone, o.orderDate, o.comments, o.status from ida.customers c 
join ida.orders o on o.customernumber = c.customernumber where o.comments is not null and o.status = 'Cancelled'

select * from ida.productlines limit 5

select * from ida.products limit 5

select p.productName, p.productLine, pl.textDescription
from ida.products p 
join ida.productLines pl on p.productLine = pl.productLine
where p.productLine in ('Planes', 'Ships')

select e.firstname, e.jobtitle, CONCAT(m.firstname, '', m.lastname) as managername from ida.employees e 
left join ida.employees m on e.reportsto = m.employeeNumber 

select p.productname from ida.products p
left join ida.orderdetails od on p.productCode = od.productCode
where od.productCode  is null

select m.lastName as managerLastName, COUNT(e.employeeNumber) as numberOfEmployees 
from employees e 
join employees m on e.reportsTo = m.employeeNumber 
group by m.employeeNumber, m.lastName 
having count(e.employeeNumber) > 2 
order by numberOfEmployees;

select o.orderdate, c.customername, o.status, p.productname, od.quantityordered, (od.quantityordered * od.priceEach)
as productTotal, sum(od.quantityOrdered * priceEach) over (partition by o.orderNumber ) as harga, 
e.firstname as employeeFirstName
from ida.orders o 
join ida.customers c on o.customerNumber = c.customerNumber
join ida.employees e on c.salesRepEmployeeNumber = e.employeeNumber
join ida.orderdetails od on o.orderNumber  = od.orderNumber
join ida.products p on p.productCode  = od.productCode
...
from ida.products 
where buyPrice  between 50 and 75 
order by buyPrice 


// distinc = kalo udah sama ga di print, jadi kek mencari ada berapa jenis
...
CREATE TABLE Dokter_Spesialis (
    NIK_Dokter VARCHAR(20),
    ID_Spesialisasi VARCHAR(10),
    PRIMARY KEY (NIK_Dokter, ID_Spesialisasi),
    FOREIGN KEY (NIK_Dokter) REFERENCES Dokter(NIK_Dokter),
    FOREIGN KEY (ID_Spesialisasi) REFERENCES Spesialisasi(ID_Spesialisasi)
) engine = innoDB

drop table Dokter_Spesialis
...
MODIFY COLUMN FrekuensiPemakaian VARCHAR(50);


UPDATE Rekam_Medis
SET ResepObat = 'Clotrimazole 50g'
WHERE ID_Rekam_Medis = 'RM005';
...
('SP001', '3201011501700001', 'Penyakit Dalam', 'Spesialis Penyakit Dalam'),
('SP002', '3201011502750002', 'Anak', 'Spesialis Anak'),
('SP003', '3201011503720003', 'Mata', 'Spesialis Mata'),
('SP004', '3201011504680004', 'Jantung dan Pembuluh Darah', 'Spesialis Jantung'),
('SP005', '3201011505730005', 'Kulit dan Kelamin', 'Spesialis Kulit')

INSERT INTO Jadwal_Praktek (ID_Jadwal, NIK_Dokter, Hari, JamMulai, JamSelesai, MaksimalPasien, Status) VALUES
('JD001', '3201011501700001', 'Senin', '08:00:00', '12:00:00', 20, 'Aktif'),
('JD002', '3201011501700001', 'Rabu', '14:00:00', '17:00:00', 15, 'Aktif'),
('JD003', '3201011502750002', 'Selasa', '09:00:00', '13:00:00', 25, 'Aktif'),
('JD004', '3201011503720003', 'Kamis', '08:30:00', '11:30:00', 12, 'Aktif'),
('JD005', '3201011504680004', 'Jumat', '13:00:00', '16:00:00', 10, 'Aktif')

INSERT INTO Pendaftaran (ID_Pendaftaran, NIK_Pasien, ID_Jadwal, NomorAntrian, TanggalDaftar, StatusDaftar, KeluhanAwal) VALUES
('REG001', '3201012801900001', 'JD001', 1, '2024-01-15', 'Confirmed', 'nyeri di bagian dada'),
('REG002', '3201012802850002', 'JD003', 2, '2024-01-16', 'Confirmed', 'Anak demam tinggi'),
('REG003', '3201012803920003', 'JD004', 1, '2024-01-17', 'Pending', 'Mata terasa perih dan berair'),
('REG004', '3201012804880004', 'JD005', 1, '2024-01-18', 'Confirmed', 'Nyeri dada dan batuk saat beraktivitas'),
('REG005', '3201012805750005', 'JD002', 3, '2024-01-19', 'Confirmed', 'kontrol panu')

INSERT INTO Rekam_Medis (ID_Rekam_Medis, ID_Pendaftaran, Diagnosis, TindakanMedis, ResepObat) VALUES
('RM001', 'REG001', 'ISPA (Infeksi Saluran Pernapasan Atas)', 'Pemeriksaan fisik, pengukuran suhu', 'Paracetamol, OBH'),
('RM002', 'REG002', 'Demam pada anak', 'Pemeriksaan fisik anak, konseling orang tua', 'Paracetamol sirup anak'),
('RM004', 'REG004', 'Hipertensi stage 1', 'Pemeriksaan EKG, konseling gaya hidup', 'Amlodipine 5mg'),
('RM005', 'REG005', 'Panu dengan kulit terkelupas', 'Pemeriksaan fisik', 'Salep anti jamur tropikal 50 ml');

drop  table 

UPDATE Rekam_Medis
SET ResepObat = 'Clotrimazole'  
WHERE ID_Rekam_Medis = 'RM005'
...
('RO001', 'RM001', 'Paracetamol', '500mg', '3x sehari', '5 hari', 15, 'Diminum setelah makan'),
('RO002', 'RM001', 'OBH', '15ml', '3x sehari', '5 hari', 1, 'Kocok sebelum diminum'),
('RO003', 'RM002', 'Paracetamol Sirup', '5ml', '3x sehari jika demam', '3 hari', 1, 'Berikan jika suhu > 38°C'),
('RO004', 'RM004', 'Amlodipine', '5mg', '1x sehari', '30 hari', 30, 'Diminum pagi hari'),
('RO005', 'RM005', 'Clotrimazole', '50 ml', '2x sehari', '30 hari', 60, 'Dioles dalam keadaan kering')

describe resep_obat

INSERT INTO Pembayaran (ID_Pembayaran, ID_Pendaftaran, RincianBiaya, TanggalBayar, MetodeBayar) VALUES
('PAY001', 'REG001', 150000.00, '2024-01-15', 'Cash'),
('PAY002', 'REG002', 200000.00, '2024-01-16', 'BPJS'),
('PAY004', 'REG004', 350000.00, '2024-01-18', 'Transfer'),
('PAY005', 'REG005', 175000.00, '2024-01-19', 'Kartu Kredit')
